...
export default class Timeline extends Core {
  ...
  constructor(container, items, groups, options) {
    ...
    if (
      !(Array.isArray(groups) || groups instanceof DataSet || groups instanceof DataView) &&
    ...
    this.on("changed", () => {
      ...
      if (
        !me.initialDrawDone &&
        (me.initialRangeChangeDone || (!me.options.start && !me.options.end) || me.options.rollingMode)
      ) {
