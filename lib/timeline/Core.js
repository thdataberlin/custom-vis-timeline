...
class Core {
  ...
  _create(container) {
    ...
    function onMouseScrollSide(event) {
      ...
      //  leave if we will have a problem with implementing of new features
      //  possibly was invented for dragging of groups earlier
  ...
  _onDrag(event) {
    if (!event) return
    // fix scrolling with scrollbar on firefox
    if (event.firstTarget && event.firstTarget.className === "vis-panel vis-left vis-vertical-scroll") {
      return
    }
